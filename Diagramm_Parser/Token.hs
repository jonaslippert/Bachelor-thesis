  ...
  where
    ...
    posToken texState pos _ s | start == "\\begin{tikzcd}" = toks
      where
        ...
        toks = posToken texState (advancePos pos (Text.concat [start, Text.replicate ((Text.length diagram) - (Text.length diagram')) "d", end])) WhiteSpaceBefore (Text.append diagram' rest'')

    -- Ignore texCenter
    posToken texState pos _ s | start == "\\begin{center}" = toks
      where
        (start, rest) = Text.splitAt 14 s
        toks = posToken texState (advancePos pos start) WhiteSpaceBefore rest

    posToken texState pos _ s | start == "\\end{center}" = toks
      where
    ...
    posToken texState pos whitespaceBefore s | useTex && hd == "\\\\" = toks
      where
...
--test = tikzcdP "A \\ar{r}{f} \\ar{d}[swap]{h} & B \\ar{d}{g} \\\\ C \\ar{r}{i} & D"
--test1 = tokenize InsideForthelEnv SAD.Core.SourcePos.noSourcePos $ Text.pack "\\begin{Lemma} \\begin{center} \\begin{tikzcd} A \\ar{r}{f} \\ar{d}[swap]{h} & B \\ar{d}{g} \\\\ C \\ar{r}{i} & D \\end{tikzcd} \\end{center} \\end{Lemma}"
